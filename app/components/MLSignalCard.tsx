...
interface Explanation {
  ...
  volatility: string;
  ai_analysis?: string; // For AI model explanations
}
...
export default function MLSignalCard({ prediction, modelUsed = "svm" }: Props) {
  ...
  return (
    <div className="bg-[#1E2130] border border-[#2E3245] rounded-xl p-5">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h3 className="text-lg font-semibold">
            {isAIModel ? "🤖 AI Prediction" : "ML Prediction"}
          </h3>
          <p className="text-xs text-gray-400 mt-1">
            Model: {getModelDisplayName(modelUsed)} {isAIModel && "via Groq"}
          </p>
        </div>
        <span
      ...
      {lime_features && lime_features.length > 0 && (
        <div className="mb-4">
      ...
      <div className="border-t border-[#30363D] pt-3 space-y-1.5">
        ...
        {isAIModel && explanation.ai_analysis && (
          <div className="text-xs">
            <div className="text-[#8B949E] mb-2">AI Analysis</div>
            <div className="text-[#C9D1D9] bg-[#161B22] p-3 rounded-md text-xs leading-relaxed">
